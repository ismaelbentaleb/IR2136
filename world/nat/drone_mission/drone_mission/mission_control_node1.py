...
def takeoff():
    print(f"Despegando a {FIXED_ALTITUDE} metros...")
    lat, lon, _ = get_current_location()
    if lat is None:
        print("ERROR: no se pudo obtener ubicación actual, abortando takeoff")
        return
    connection.mav.command_long_send(
        connection.target_system,
        connection.target_component,
        ...
        0,
        0,0,0,0,
        lat, lon, FIXED_ALTITUDE
    )
    print("Comando de takeoff enviado!")
...
def main():
    rclpy.init()
    node = rclpy.create_node('mission_control_node')
